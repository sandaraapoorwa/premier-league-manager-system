...
// Cache so we don't re-fetch the same player
const imageCache = {};

function PlayerCard({ p, i }) {
  const group = posGroup(p.position);
  const pc = posColor[group];
  const rc = ratingColor(p.rating ?? 0);
  const [imgUrl, setImgUrl] = useState(imageCache[p.name] ?? null);
  const [imgFailed, setImgFailed] = useState(false);

  useEffect(() => {
    if (imgUrl || imgFailed || imageCache[p.name] === "none") return;

    const fetchImage = async () => {
      try {
        const slug = encodeURIComponent(p.name.replace(/ /g, "_"));
        ...
        if (url) {
          imageCache[p.name] = url;
          setImgUrl(url);
        } else {
          imageCache[p.name] = "none";
          setImgFailed(true);
        }
      } catch {
        imageCache[p.name] = "none";
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
        ...
        <div style={{
          ...
          {imgUrl && !imgFailed ? (
            ...
            <span style={{
              ...
              fontSize: 16, color: pc.text, letterSpacing: "1px",
            }}>
              {initials(p.name)}
            </span>
          )}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            fontSize: 16, fontWeight: 700, color: "#c8d4e0",
            letterSpacing: "0.3px",
          ...
          <div style={{
            fontFamily: "'Barlow Condensed', sans-serif",
            fontSize: 13, color: "#3d5060", letterSpacing: "0.3px",
            marginTop: 2,
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          }}>
            {p.club ?? "Unknown Club"}
        ...
        <div style={{
          flexShrink: 0,
          fontFamily: "'Bebas Neue', sans-serif",
          fontSize: 28, color: rc, lineHeight: 1, letterSpacing: "1px",
        }}>
          {p.rating ?? "—"}
        </div>
      </div>
      ...
      <div style={{
        ...
        <span style={{
          ...
          background: pc.bg, border: `1px solid ${pc.border}`,
          color: pc.text, borderRadius: 6, padding: "3px 9px",
        }}>
          {p.position ?? "N/A"}
        </span>

        <div style={{
          flex: 1, marginLeft: 12, height: 3,
...
export default function PlayersPage() {
  ...
  const visible = players
    .filter((p) => {
      ...
      if (pos === "DEF") return ["RB","CB","LB"].includes(p.position);
      if (pos === "MID") return ["CDM","CM","CAM"].includes(p.position);
      if (pos === "FWD") return ["RW","LW","ST"].includes(p.position);
      return false;
    })
    .filter((p) =>
      !search ||
  ...
  return (
    <div style={{
      ...
      <header style={{ marginBottom: 28 }}>
        ...
        <p style={{
          fontFamily: "'Barlow Condensed', sans-serif",
          fontSize: 15, color: "#3d5060", letterSpacing: "0.5px", marginTop: 6,
        }}>
          {players.length} players across all clubs
        </p>
      </header>
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            borderRadius: 10, padding: "9px 16px", color: "#eef0f6",
            fontFamily: "'Barlow Condensed', sans-serif",
            fontSize: 15, letterSpacing: "0.4px", outline: "none", width: 220,
          }}
        />
        ...
        <select
          ...
          style={{
            background: "rgba(255,255,255,0.04)",
            border: "1px solid rgba(255,255,255,0.09)",
            borderRadius: 10, padding: "9px 14px", color: "#7a8aa0",
            fontFamily: "'Barlow Condensed', sans-serif",
            fontSize: 14, letterSpacing: "0.4px", outline: "none", cursor: "pointer",
          }}
        >
        ...
        <span style={{
          fontFamily: "'Barlow Condensed', sans-serif",
          fontSize: 13, color: "#3d5060", letterSpacing: "0.4px", marginLeft: "auto",
        }}>
      ...
      {!loading && visible.length === 0 && !error && (
        <p style={{
          fontFamily: "'Barlow Condensed', sans-serif",
          fontSize: 16, color: "#2a3a48", letterSpacing: "0.5px",
